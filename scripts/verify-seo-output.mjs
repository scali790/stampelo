...
for (const route of ["/api/", "/account", "/admin", "/download"]) {
  assert(robotsTxt.includes(`Disallow: ${route}`), `robots.txt disallows ${route}`);
}
assert(robotsTxt.includes("Sitemap: https://www.stampelo.com/sitemap.xml"), "robots.txt references canonical sitemap");

const sitemap = fs.readFileSync(path.join(distDir, "sitemap.xml"), "utf8");
for (const route of ["/account", "/admin", "/download"]) {
  assert(!sitemap.includes(`https://www.stampelo.com${route}`), `sitemap excludes ${route}`);
}
...
function assert(condition, message) {
  if (!condition) {
    console.error(`[seo] FAILED: ${message}`);
    process.exit(1);
  }
  console.log(`[seo] ok: ${message}`);
}
